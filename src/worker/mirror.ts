...
export class AutomatonMirror {
    ...
    constructor(private readonly registry: OpenedContract<KronosRegistry>) {}
    ...
    async ensureFresh(): Promise<void> {
        const count = await this.registry.getActiveAutomatonCount();
    ...
    async refresh(activeCount?: bigint): Promise<void> {
        const count =
            activeCount ?? (await this.registry.getActiveAutomatonCount());
        const next: Address[] = [];
        for (let i = 0; i < Number(count); i++) {
            const addr = await this.registry.getAutomatonAt(i);
            if (addr === null) {
                throw new Error(
