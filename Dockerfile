...
# `kronos/sdk/` and `forgeton/sdk/` trees are visible.
#
#   docker buildx build \
...
# that moment, AND because fortuna/sdk's devDeps file: forgeton +
# atlas — those must be built (dist/ populated) before fortuna's
# install runs.
COPY forgeton/sdk   ./forgeton/sdk
COPY atlas/sdk      ./atlas/sdk
COPY kronos/sdk     ./kronos/sdk
COPY fortuna/sdk    ./fortuna/sdk
...
# kronos/sdk/node_modules on disk causes pnpm to snapshot a second copy
...
# survive the rm.
RUN cd forgeton/sdk && pnpm install --prefer-offline && pnpm run build && rm -rf node_modules
RUN cd atlas/sdk    && pnpm install --prefer-offline && pnpm run build && rm -rf node_modules
RUN cd kronos/sdk   && pnpm install --prefer-offline && pnpm run build && rm -rf node_modules
RUN cd fortuna/sdk  && pnpm install --prefer-offline && pnpm run build && rm -rf node_modules
