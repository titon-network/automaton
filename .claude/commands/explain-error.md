...
1. If the input is a bare number (e.g. `162`) or contains `exit code N`, look it up via `explainExitCode(n)` — the source tables are in `kronos-sdk` (codes 100-119), `forgeton-sdk` (160-182), and `@ton/core`-equivalent TVM (1-100). Read `src/errors/explain.ts` for the dispatch logic.
