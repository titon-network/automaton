...
import type { JobData, KronosRegistry, RegistryConfigReply } from '@titon-network/kronos-sdk';
import {
...
import { decide, type Decision, type DecisionReason } from './decide';

// ===== Observability contracts =====
//
// `WorkerLogger` + `WorkerCounters` are the cross-cutting interfaces
// the daemon and worker share. They live here (rather than a separate
// observability/ module) because the worker is their primary owner —
// `runWorkerCycle` is the reason these contracts exist.
...
export interface WorkerCounters {
    incrementExecuteAttempt(reason: DecisionReason): void;
    incrementExecuteSuccess(reason: DecisionReason): void;
    incrementExecuteFailure(reason: DecisionReason, errorClass: ExecuteFailureClass): void;
    incrementSkip(reason: DecisionReason): void;
...
export interface WorkerDeps {
    /** Kronos registry handle — opened in the kronos product module. */
...
export async function runWorkerCycle(deps: WorkerDeps): Promise<WorkerCycleResult> {
    ...
    try {
        ...
        [registryConfig, jobCount, registryPaused] = await Promise.all([
            deps.registry.getConfig(),
            deps.registry.getJobCount(),
            deps.registry.getIsPaused(),
        ]);
    } catch (err) {
    ...
    const jobs = await fetchJobsBounded(deps.registry, jobCount, concurrency, logger);
...
async function fetchJobsBounded(
    ...
    const workers = Array.from({ length: Math.min(concurrency, ids.length) }, async () => {
        while (true) {
            ...
            const jobId = ids[idx]!;
            const job = await safeFetchJob(registry, jobId, logger);
...
async function safeFetchJob(
    ...
    try {
        return await registry.getJob(jobId);
    } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
...
export const defaultSubmitExecute: SubmitExecuteFn = async (
    ...
    const sender = senderFor(deps.client, deps.wallet);
    await sendAndConfirm(
        deps.client,
        deps.wallet,
        () => deps.registry.sendExecute(sender, { value: config.minGasReserve, jobId }),
        {
            verify: async () => {
