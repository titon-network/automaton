...
import { blsPublicKeyG2 as blsPublicKeyG2Fn } from '@titon-network/fortuna-sdk';
...
async function loadBlsContext(groupIdRaw: string): Promise<BlsContext> {
    ...
    // Unlock BLS to get the secret — `register` needs it to derive the
    // 96-byte G2 sibling (`pkShareG2`) that Atlas's RegisterBlsShare now
    // requires alongside the 48-byte G1 pkShare. Zero the secret as soon
    // as G2 is derived to minimise its in-memory lifetime.
    const blsSecret = unlockBlsKeystore(blsKeystore, password);
    const blsPublicKeyG2Hex = Buffer.from(blsPublicKeyG2Fn(blsSecret)).toString('hex');
    blsSecret.fill(0);
    ...
    return {
        ...
        blsPublicKeyG2: blsPublicKeyG2Hex,
        atlasConfig,
        existing,
        groupId,
    };
    ...
    const result = await sendAndConfirm(
        ...
        () =>
            ctx.atlas.sendRegisterBlsShare(ctx.sender, {
                ...
                groupId: ctx.groupId,
                pkShare,
                pkShareG2: Buffer.from(ctx.blsPublicKeyG2, 'hex'),
            }),
        {
            verify: async () => {
    ...
    if (groupKey === null) {
        ...
        throw new Error(
            'no group key published at Atlas yet (atlas.getGroupKey(0) === null). ' +
                ...
                '../atlas/sdks/typescript/skills/atlas-operator-register-share.md §"Solo-mode flow".',
        );
    }
