...
import { PoolRejectedError } from '../src/chain';
...
function buildFakeRuntime(overrides: FakeRuntimeOverrides = {}): {
    registry: unknown;
    client: unknown;
    pool: unknown;
} {
...
describe('runWorkerCycle', () => {
    it('records decisions for every job and reports jobCount', async () => {
        ...
        const result = await runWorkerCycle({
            ...
            wallet: fakeWallet(),
            mirror: new AutomatonMirror(runtime.registry as never),
            inFlight: new Set(),
            nowSec: () => NOW_SEC,
            submitExecute: async () => {
...
describe('AutomatonMirror', () => {
    it('refreshes on first ensureFresh call', async () => {
        ...
        const mirror = new AutomatonMirror(runtime.registry as never);
    ...
    it('skips the per-slot refetch when count is unchanged', async () => {
        ...
        const mirror = new AutomatonMirror(registry as never);
