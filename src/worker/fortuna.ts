...
//
// Solo-operator path (t=1, n=1):
//   Atlas is bootstrapped with a single operator whose pkShare == groupPk.
//   BLS "aggregation" is the identity — one partial signature verifies as
//   the aggregate. This sub-phase proves the crypto + on-chain plumbing
//   end-to-end; multi-operator threshold aggregation adds a peer-
//   coordination layer (out of scope here).
//
...
export class FortunaWorker {
    ...
    async tick(): Promise<{ attempts: number; successes: number; failures: number; skipped: number }> {
        ...
        for (const key of keys) {
            ...
            // Dequeue so we don't keep trying every tick.
            if (now >= req.deadline) {
    ...
    private async submitOne(req: PendingFortunaRequest): Promise<boolean> {
        ...
        // Solo-operator path: the partial IS the aggregate. When multi-
        // operator lands, swap this for the t-of-n exchange + aggregation.
        const aggSig = partial;
