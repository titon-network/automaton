...
**Helping a user deploy?** [`docs/deploy.md`](docs/deploy.md) is the decision tree — "user says X" → path → verbatim commands. Five paths covered (playground, local testnet, Lightsail, self-hosted Docker, self-hosted systemd) plus a common-errors table. Read that first if the question is "how do I get this running".

**Helping a user enable Fortuna VRF?** Prerequisite chain: stake at ForgeTON → Atlas admitted as a ForgeTON consumer (one-time, ForgeTON-owner-driven; see `../atlas/sdk/skills/atlas-deploy.md` §Step 2) → `automaton bls keygen` → `automaton bls register`. The middle step is the one operators trip on — `bls register` reverts with `OperatorNotFound (120)` until Atlas is in ForgeTON's consumer set.
