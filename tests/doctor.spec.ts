...
describe('runChecks', () => {
    ...
    it('emits skip rows for every chain check when the network has no live deployment', async () => {
        // Mainnet has no Kronos deployment yet — buildChainRuntime throws
        // DeploymentNotAvailableError; doctor catches it and emits one
        // skip row per declared chain check (so operators see exactly
        // what *would* have run on a live network).
        ...
        for (const name of chainCheckNames) {
            ...
            expect(c.detail).toMatch(/mainnet/i);
        }
    });
