...
import { blsPublicKeyG2 } from '@titon-network/fortuna-sdk';
import {
...
async function bootstrapFortunaPlayground(deps: {
    ...
    // Atlas now also requires the 96-byte G2 sibling for both register +
    // publish (pairing-based ChallengeReveal gate). In solo-mode the
    // group's G2 IS the operator's G2.
    const pkShareG2 = Buffer.from(blsPublicKeyG2(blsSecret));
    ...
    await harness.registerBlsShare({ operator: automaton, pkShare, pkShareG2 });
    ...
    await harness.publishGroupKey({ groupPk: pkShare, groupPkG2: pkShareG2, threshold: 1, memberCount: 1 });
