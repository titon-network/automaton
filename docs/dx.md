...
| `automaton bls <keygen\|pubkey\|register\|deregister> [--force\|--group-id <id>]` | BLS12-381 identity for Fortuna VRF + Themis reveals (one shared keystore — Atlas group secret signs both). `keygen` produces `~/.titon/automaton/bls.enc` (same password as wallet by default); `pubkey` reads the plaintext G1 pkShare without unlocking; `register` / `deregister` submit `RegisterBlsShare` / `DeregisterBlsShare` to Atlas. `register` reverts with `OperatorNotFound (120)` if Atlas isn't yet admitted as a ForgeTON consumer. |
