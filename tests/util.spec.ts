...
import { existsSync, mkdtempSync, readdirSync, readFileSync, rmSync, statSync, symlinkSync, writeFileSync } from 'fs';
...
describe('atomicWriteFile', () => {
    ...
    it('refuses to follow a pre-existing symlink at the tmp path (TOCTOU defense)', () => {
        // Pre-attack: an attacker creates `${path}.${pid}.tmp` as a symlink
        // to a sensitive target. Without O_EXCL, atomicWriteFile would
        // follow the symlink and scribble on the target. With O_EXCL ('wx')
        // the open fails with EEXIST and we abort.
        const target = join(dir, 'target.json');
        const sensitive = join(dir, 'sensitive.txt');
        writeFileSync(sensitive, 'untouched-secret');
        const tmp = `${target}.${process.pid}.tmp`;
        symlinkSync(sensitive, tmp);

        expect(() => atomicWriteFile(target, 'attacker-payload')).toThrow(/EEXIST/);

        // Sensitive target unchanged (no follow).
        expect(readFileSync(sensitive, 'utf8')).toBe('untouched-secret');
        // Real target was never written.
