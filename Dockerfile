...
# Disable pnpm 10.x's supply-chain minimum-release-age policy for the
# whole builder stage. We publish + consume our own @titon-network/*
# SDKs same-day; the default cutoff would block every release. Setting as
# an env var rather than a CLI flag because pnpm internally re-invokes
# itself for deps-status checks during `pnpm run` and those re-invocations
# don't inherit the CLI flag.
...
COPY automaton/package.json automaton/pnpm-lock.yaml automaton/tsconfig.json automaton/.npmrc ./automaton/
COPY automaton/src      ./automaton/src
COPY automaton/contrib  ./automaton/contrib

WORKDIR /src/automaton
# Inline --config.minimum-release-age=0 because pnpm 11 in this container
# does not honor .npmrc at policy-check time. We also bypass
# `pnpm run build` (which re-invokes pnpm install internally and reapplies
# the policy) by calling tsc directly via node_modules/.bin.
RUN pnpm install --prefer-offline --frozen-lockfile --ignore-scripts \
    --config.minimum-release-age=0
RUN ./node_modules/.bin/tsc -p tsconfig.json
RUN chmod +x dist/cli/index.js

# Prune dev-deps so stage 2 only carries prod runtime. CLI flag again
# because prune also runs the supply-chain policy check on the lockfile.
