...
```bash
pnpm install                       # resolves all deps incl. file: SDK snapshots; runs postinstall preflight (warn-only)
...
pnpm dev <args>                    # ts-node source-run; no build needed (sub-second iteration). NB: use `pnpm dev` NOT `pnpm run dev --` (commander chokes on the `--` separator)
pnpm run preflight                 # ad-hoc sibling-SDK landmine scan (same checks as postinstall + jest)
pnpm run verify                    # build + test + smoke — the single gate an agent runs before claiming "done"
pnpm run sync:sdks                 # rebuild ../forgeton/sdk + ../kronos/sdk + force reinstall
```

### Slash commands for LLM-driven work

The repo ships `.claude/commands/` with task-shaped recipes. Invoke them from a Claude Code session:

- `/verify` — run `pnpm run verify` and summarise
- `/navigate <task>` — resolve "I want to X" → exact file via the navigator table below
...
- `/new-handler <name>` — add an `EventHandler` with debounced side-effects
- `/new-config-field <name>:<type>` — schema + env overlay + docs + test
- `/preflight` — run the sibling-SDK landmine scan
- `/explain-error <code-or-message>` — look up a TVM exit code or typed error
- `/dev <args>` — run the CLI via ts-node, no build needed
...
Phases D.1 → D.15 are complete; full per-phase history lives in [`docs/phases.md`](docs/phases.md). Verify the current surface with `pnpm run verify` (build + test + smoke).
