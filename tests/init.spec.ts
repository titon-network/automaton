...
describe('automaton init (non-interactive)', () => {
    ...
    it('honours --rpc-url + --rpc-api-key, writing them into config.endpoints[0]', async () => {
        await runInit(
            {
                ...flags(),
                rpcUrl: 'https://my.toncenter.example/api/v2/jsonRPC',
                rpcApiKey: 'secret-key',
            },
            FAST_KDF,
        );
        const cfg = loadConfig(configPath());
        expect(cfg.endpoints).toEqual([
            { url: 'https://my.toncenter.example/api/v2/jsonRPC', apiKey: 'secret-key' },
        ]);
    });

    it('honours --rpc-url alone (no apiKey emitted)', async () => {
        await runInit(
            { ...flags(), rpcUrl: 'http://my-self-hosted-ton-http-api.local:8081/api/v2/jsonRPC' },
            FAST_KDF,
        );
        const cfg = loadConfig(configPath());
        expect(cfg.endpoints).toHaveLength(1);
        expect(cfg.endpoints[0]!.url).toBe(
            'http://my-self-hosted-ton-http-api.local:8081/api/v2/jsonRPC',
        );
        expect(cfg.endpoints[0]!.apiKey).toBeUndefined();
    });

    it('honours --rpc-api-key alone — keeps the network default URL, attaches the key', async () => {
        await runInit({ ...flags(), rpcApiKey: 'pro-tier-key' }, FAST_KDF);
        const cfg = loadConfig(configPath());
        expect(cfg.endpoints).toHaveLength(1);
        // Default testnet URL stays put.
        expect(cfg.endpoints[0]!.url).toBe('https://testnet.toncenter.com/api/v2/jsonRPC');
        expect(cfg.endpoints[0]!.apiKey).toBe('pro-tier-key');
    });

    it('rejects a malformed --rpc-url', async () => {
        await expect(
            runInit({ ...flags(), rpcUrl: 'not-a-url' }, FAST_KDF),
        ).rejects.toThrow(/--rpc-url: invalid endpoint/);
    });

    it('treats --rpc-api-key="" as "no key" (no apiKey field written to config)', async () => {
        await runInit({ ...flags(), rpcApiKey: '' }, FAST_KDF);
        const cfg = loadConfig(configPath());
        expect(cfg.endpoints).toHaveLength(1);
        expect(cfg.endpoints[0]!.apiKey).toBeUndefined();
    });

    it('falls back to the network default endpoint when no rpc flags are passed', async () => {
        await runInit(flags(), FAST_KDF);
        const cfg = loadConfig(configPath());
        expect(cfg.endpoints).toEqual([
