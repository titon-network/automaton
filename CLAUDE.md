...
- **Phoebe** (opt-in via `products.phoebe` + `config.phoebe.feeds`): heartbeat-push a BLS-attested Merkle snapshot of the configured price feeds every `pushIntervalMs` window. The operator builds a `PhoebeMerkleTree` from `config.phoebe.feeds[]`, BLS-signs `computeSnapshotHash(phoebeAddress, timestamp, root)` (where `timestamp` is rounded to the window boundary for deterministic multi-op aggregation), and submits `PushSnapshot`. Same BLS secret as Fortuna/Themis. Solo when `config.phoebe.peers` is empty; multi-op via share-exchange on port 9092 (mirrors fortuna's :9091). Feeds may be **static** (`{feedId, mantissa, expo, confBps}` — operator hand-sets price; the only path with byte-identical roots across operators in multi-op) or **dynamic** (`{feedId, sources: [{name, symbol}, ...]}` — production: the aggregator pulls live prices from CEX websockets (Binance / Coinbase / Kraken) + STON.fi DEX TWAP fallback, takes the median across fresh ticks, derives `confBps` from the half-spread). See `src/products/phoebe-sources/` for the v2 price-source layer. **Multi-op + dynamic is best-effort**: per-operator tick sets are not identical (one might have received a Coinbase tick the others haven't), so medians can drift → BLS partials fail to aggregate → leader-grace fallback fires and one operator submits alone (losing the threshold-attestation guarantee for that window). Strict t=n attestation requires static feeds today; v2.1 will add a propose/ratify coordination round for deterministic multi-op dynamic feeds.

Design goal: *super easy to launch, hassle-free, production-ready*. `automaton init` → `automaton stake register` → `automaton run` is the Kronos onboarding flow; add `automaton bls keygen` → `automaton bls register` when enabling Fortuna. Themis adds nothing further on the operator side beyond `products.themis: true` + a chamber list — the same BLS share signs reveals. Phoebe likewise reuses `bls.enc` — flip `products.phoebe: true` + populate `config.phoebe.feeds[]`. `automaton doctor` / `automaton status` for diagnostics.
...
| Change the Phoebe push path | `src/worker/phoebe.ts` | `PhoebeWorker.eventHandler()` tracks paused / group-epoch state from Phoebe events; `.tick()` rounds `now` to a `pushIntervalSec` window boundary, builds a `PhoebeMerkleTree` from `config.phoebe.feeds[]`, computes `computeSnapshotHash(phoebeAddress, timestamp, root)`, signs, and submits `PushSnapshot`. Two modes: solo (peers empty, partial == aggregate) and multi-op additive `t=n` (peers non-empty → sign + broadcast via `src/daemon/share-exchange-phoebe.ts` on port 9092 → wait for `n` partials → leader-grace → `aggregateSignatures` from phoebe-sdk → submit). Window-rounded timestamps make the multi-op message deterministic across operators without out-of-band consensus, assuming clock sync within `maxDriftSec` (server-side reject bound, default 60s) AND identical `feeds[]` config (operators MUST coordinate the feed list off-chain). |
| Change the Phoebe price-source layer (CEX adapters / aggregator) | `src/products/phoebe-sources/` | `types.ts` — `PriceSource` / `Tick` / `FeedConfig`. `manager.ts` — `PriceFeedManager` owns the latest-tick cache + median aggregator (drops stale ticks older than `maxStaleMs`, gates on `minSources`, normalises to a common expo, derives `confBps` from half-spread). `ws-base.ts` — hardened WebSocket adapter base class: per-attempt connect timeout, silence-timeout reconnect, pong-deadline tracking, equal-jitter exponential backoff with stable-uptime reset. `binance.ts` / `coinbase.ts` / `kraken.ts` — one CEX adapter each, each ~80 lines (subscribe message + `handleMessage` parser). `stonfi-twap.ts` — DEX poll-based fallback (stub fetcher in v2; full pool-reader in v2.1). `index.ts` — adapter registry + `buildAdapters(names, logger)`. Tests: `tests/phoebe-aggregator.spec.ts` (pure logic) + `tests/phoebe-source-adapter.spec.ts` (real `ws` server, parses Binance/Coinbase/Kraken-shaped messages, reconnect after server-side close). |
| Change the Phoebe ProductModule wiring | `src/products/phoebe.ts` | Resolves atlas + phoebe via SDK constants or `config.phoebe.atlasAddress` / `config.phoebe.phoebeAddress` overrides. Event source is `phoebe`. `bootstrapWorker` is gated on `bls.enc` presence (Phoebe reuses the Fortuna/Themis BLS secret); when `config.phoebe.peers` is populated it also stands up a `PhoebeShareCache` + share-exchange HTTP server on port 9092 with an Atlas-backed pkShare lookup. Push-error hint: pass `origin: 'phoebe'` to `sendAndConfirm` so phoebe exit codes route to the right explainer. |
| Change the Phoebe public read endpoint | `src/daemon/share-exchange-phoebe.ts` (route handler) + `src/worker/phoebe.ts` (snapshotRef writer) | The multi-op share-exchange server also serves `GET /phoebe/v1/snapshot` — a no-auth, CORS-open JSON view of the last verified push's leaves. DApps fetch this from any operator, reconstruct the merkle root locally, and compare against `phoebe.lastSnapshot.root` on-chain → catches lying operators via hash mismatch, no trust assumption. Solo operators don't run the server, so no public endpoint there (intentional — solo isn't part of a decentralized indexer set). Wire shape: `PhoebeSnapshotRef` is a mutable holder shared between the worker (writes after `lastPushSec = opts.timestamp`) and the server (reads inside the GET handler). Exposing publicly requires `config.phoebe.shareExchangeHost: "0.0.0.0"` + SG ingress on :9092 (handled by `contrib/aws/ec2/main.tf:aws_vpc_security_group_ingress_rule.phoebe_public_read`). |
...
| Cut a release | [`RELEASE.md`](RELEASE.md) | Full pipeline: bump three files in lockstep (package.json + both region tfs), tests, npm tarball pack, docker buildx multi-arch push to ECR, `pnpm publish --otp`, terraform apply EU + US sequentially, verify each, dapp-dev smoke. Captures every gotcha hit on 0.12.0 → 0.14.0 ships (pnpm supply-chain policy, `pnpm run build` re-invoking install, qemu binfmt, V5R1 ghost-tx, snap docker socket, instance-replace downtime). [`scripts/release.sh`](scripts/release.sh) handles the bump+test+tag prefix; the docker / SSM / terraform tail is manual per the runbook. |
...
| `phoebe push did not land within 30000ms ... V5R1 ghost-tx — DEPLOY_CORE §5.3` | Wallet balance dropped below `value + storage reserve`; V5R1 silently dropped the action phase while seqno still advanced | Top up the wallet address printed in the error (≥ 5 TON gives runway for weeks at ~30s push cadence). The 0.14.0 verify replaced the old false-positive seqno-check with a `phoebe.lastSnapshotTime` poll — see [`RELEASE.md`](RELEASE.md) §G4. The metric `automaton_wallet_balance_ton` should alert below 1 TON to catch the drain class. The same V5R1 pattern affects any owner ForceSync wallet — see §G9. |
...
Four products supported out of the box:
...
- **Phoebe** (opt-in via `config.products.phoebe` + `config.phoebe.feeds`): heartbeat-push a BLS-attested Merkle snapshot of the configured price feeds every `pushIntervalMs` window. Solo + multi-op both ship in v1 (testnet runs solo with `peers: []`, mainnet populates `peers` for additive `t=n`). Feeds are either **static** (`{feedId, mantissa, expo, confBps}`) or **dynamic** (`{feedId, sources: [{name, symbol}, ...]}`) — dynamic feeds pull live prices from CEX websockets (Binance + Coinbase + Kraken) plus a STON.fi DEX TWAP fallback, median-aggregate per push window, derive `confBps` from the half-spread. Source layer: `src/products/phoebe-sources/`. **Combo caveat**: multi-op + dynamic feeds is best-effort (operators' tick sets diverge → medians drift → partials don't aggregate → leader-grace fallback has one operator submit alone). Strict t=n attestation = use static feeds in multi-op today.

Operator-adverse conditions (slashed, pruned, missed assigned slot, group-key rotation, registry/pool/fortuna/themis/phoebe pause flips, code upgrades, opt-in changes, factory-driven chamber-bytecode upgrades) surface at the appropriate log level via the built-in event handlers — the operator doesn't grep tx history to find out what happened.
