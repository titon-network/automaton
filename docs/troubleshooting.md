...
**Fix:** the error message tells you which direction — either upgrade `@titon/automaton` (contract is newer than you) or wait / redeploy (you're newer than the contract). See `automaton status` to see which side is ahead.
...
**Fix:** read the logged stack to find the bug; file an issue at [github.com/titon/titon/issues](https://github.com/titon/titon/issues) with the stack + your config (redact secrets, though pino already redacts the common field names).
...
- Open an issue at [github.com/titon/titon/issues](https://github.com/titon/titon/issues) with:
  - `automaton --version`
