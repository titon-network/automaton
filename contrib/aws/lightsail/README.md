...
> **For mainnet, use the [EC2 module](../ec2/) instead.** Lightsail's keystore-in-TF-state posture is acceptable for testnet only. The team's mainnet operators run on EC2 with SSM-backed secrets + multi-op Fortuna support.
...
For mainnet, **prefer the EC2 module** at [`../ec2/`](../ec2/) — same Terraform shape, but secrets ride in SSM Parameter Store (KMS-encrypted, IAM-scoped), the share-exchange port is wired for multi-op Fortuna, and operator shell access is via SSM Session Manager (no inbound port). Full reference at [`../ec2/README.md`](../ec2/README.md); multi-op protocol + operator-setup at [`../../../docs/multi-op-fortuna.md`](../../../docs/multi-op-fortuna.md). The Lightsail module stays the cheapest testnet-canary path.
