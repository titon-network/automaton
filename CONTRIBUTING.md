...
Thanks for looking at `@titon-network/automaton`. This file is the 60-second orientation; detailed context lives in the other docs.
...
If you edit a sibling SDK (`../kronos/sdks/typescript/` or `../forgeton/sdks/typescript/`), run `pnpm run sync:sdks` to rebuild + re-snapshot. Never `pnpm install` inside a sibling SDK — the preflight script will refuse to start until you `rm -rf` the nested `node_modules/`.
