...
import { KronosWorker, mirrorPatchHandler } from '../src/products/kronos';
...
async function runTicks(params: {
    ...
    const kronosWorker = params.kronosWorker ?? makeKronosWorker(params.harness, params.me, metrics);
    const handlers = params.handlers ?? [];
    let checkpointState = params.checkpointState ?? emptyCheckpointState();

    // Sources = baseline (pool) + kronos product's registry stream (the
    // kronos product owns the registry source, so sandbox tests synthesize
    // it manually here without instantiating the full ProductModule wiring).
    const sources = [
        ...baselineEventSources(params.harness.runtime),
        // Synthesize the kronos registry source manually with the real
        // SDK decoder so dispatched-event counters increment correctly.
        kronosRegistrySource(params.harness),
    ];
    for (let i = 0; i < params.ticks; i++) {
        checkpointState = await tickOnce({
            ...
            checkpointState,
            productWorkers: { kronos: kronosWorker },
        });
    }

    return { metrics, kronosWorker, checkpointState };
}

/** Build a KronosWorker wired to the sandbox harness — submitExecute
 *  routes through the harness's treasury sender, nowSec follows simulated
 *  blockchain time, and counters land in the supplied DaemonMetrics. */
function makeKronosWorker(
    ...
    const worker = new KronosWorker({
        ...
        submitExecute: options.submitExecute ?? harness.submitExecuteVia(me),
        nowSec: () => harness.nowSec(),
        jobFetchConcurrency: 2,
        ...(options.mirror !== undefined ? { mirror: options.mirror } : {}),
    });
    // Tests that need to pre-populate the in-flight Set (single-flight
    // collision check) inject their own.
    if (options.inFlight !== undefined) {
        for (const id of options.inFlight) worker.inFlight.add(id);
    }
    return worker;
}

/** Helper: build the standard sandbox tickOnce inputs (sources + handlers + worker). */
function makeTickInputs(
    harness: SandboxHarness,
    me: SandboxContract<TreasuryContract>,
    options: {
        handlers?: readonly EventHandler[];
    ...
    const worker =
        options.worker
        ?? makeKronosWorker(harness, me, metrics, {
            ...(options.submitExecute !== undefined ? { submitExecute: options.submitExecute } : {}),
            ...(options.mirror !== undefined ? { mirror: options.mirror } : {}),
            ...(options.inFlight !== undefined ? { inFlight: options.inFlight } : {}),
        });
    const sources = [
        ...baselineEventSources(harness.runtime),
        // Synthesize the kronos product's registry source manually so
        // sandbox tests can drain registry events without instantiating
        // the full ProductModule wiring.
        kronosRegistrySource(harness),
    ];
    return {
        metrics,
        worker,
...
function kronosRegistrySource(harness: SandboxHarness) {
    ...
    return {
        ...
        decode: (bodies: import('@ton/core').Cell[]) => {
...
describe('Integration: daemon tickOnce against sandbox', () => {
    ...
    describe('happy path — single automaton, never-executed first run', () => {
        it('executes a fresh job, emits JobExecuted, and the mirror reflects activeCount=1', async () => {
            ...
            const { metrics, kronosWorker } = await runTicks({
                harness,
                me,
                ticks: 1,
            });
            ...
            // Mirror (owned by the kronos worker) reflects the single
            // registered automaton.
            expect(kronosWorker.mirror.activeCount).toBe(1n);
            expect(kronosWorker.mirror.snapshot[0]!.equals(me.address)).toBe(true);
    ...
    describe('fallback window — claim + slash', () => {
        it('wins the reward and slashes the missed assigned automaton', async () => {
            ...
            const missedSlashCountBefore =
                (await harness.pool.getAutomaton(missed.address))?.slashCount ?? 0;
            ...
            // Registry sent a Slash → pool's slashCount on the missed
            // automaton bumped. (The on-chain drift counters were dropped at
            // the v1 launch baseline; observability lives at the event-stream
            // layer now — Argus diffs AssignedAutomatonMissed against
            // AutomatonSlashed.)
            const missedInfo = await harness.pool.getAutomaton(missed.address);
            expect(missedInfo).not.toBeNull();
            expect(missedInfo!.slashCount).toBeGreaterThan(missedSlashCountBefore);
        });
    });
    ...
    describe('mirror handler — AutomatonMirrorUpdated triggers refresh', () => {
        it('wires mirrorPatchHandler to refresh when the event drains', async () => {
            ...
            const mirror = new AutomatonMirror(harness.runtime.products.kronos!.registry as never);
            ...
            // exactly one refresh.
            const inputs = makeTickInputs(harness, me, { handlers, metrics, mirror });
            await tickOnce({
                ...
                productWorkers: { kronos: inputs.worker },
            });
    ...
    describe('self-slash webhook round-trip', () => {
        it('POSTs the webhook when this automaton is slashed in a fallback claim', async () => {
            ...
            const slashedInputs = makeTickInputs(harness, willBeSlashed, {
                handlers: [handler],
            });
            await tickOnce({
                runtime: harness.runtime,
                wallet: harness.fakeWalletFor(willBeSlashed),
                handlers: slashedInputs.handlers,
                sources: slashedInputs.sources,
                metrics: slashedInputs.metrics,
                logger: SILENT_LOGGER,
                checkpointState: emptyCheckpointState(),
                productWorkers: { kronos: slashedInputs.worker },
            });
    ...
    describe('idle — no jobs, no events', () => {
        it('runs a no-op tick without throwing and without advancing the checkpoint', async () => {
            ...
            const idleInputs = makeTickInputs(harness, me, { metrics });
            const nextState = await tickOnce({
                runtime: harness.runtime,
                wallet: harness.fakeWalletFor(me),
                handlers: idleInputs.handlers,
                sources: idleInputs.sources,
                metrics: idleInputs.metrics,
                logger: SILENT_LOGGER,
                checkpointState: initialState,
                productWorkers: { kronos: idleInputs.worker },
            });
    ...
    describe('consumer watch handler', () => {
        it('logs ConsumerUpdated when a new consumer is admitted', async () => {
            ...
            const consumerInputs = makeTickInputs(harness, me, { handlers: [handler] });
            await tickOnce({
                runtime: harness.runtime,
                wallet: harness.fakeWalletFor(me),
                handlers: consumerInputs.handlers,
                sources: consumerInputs.sources,
                metrics: consumerInputs.metrics,
                logger: SILENT_LOGGER,
                checkpointState: emptyCheckpointState(),
                productWorkers: { kronos: consumerInputs.worker },
            });
    ...
    describe('metrics wiring', () => {
        ...
        it('does not emit drain_dispatched counter when no events drained', async () => {
            ...
            await runTicks({
                ...
                kronosWorker: first.kronosWorker,
            });
    ...
    describe('execute revert — post-state verify', () => {
        it('counts a verify-failed attempt when Execute reverts (insufficient gas)', async () => {
            ...
            const breakingInputs = makeTickInputs(harness, me, {
                metrics,
            ...
            await tickOnce({
                runtime: harness.runtime,
                wallet: harness.fakeWalletFor(me),
                handlers: breakingInputs.handlers,
                sources: breakingInputs.sources,
                metrics: breakingInputs.metrics,
                logger: SILENT_LOGGER,
                checkpointState: emptyCheckpointState(),
                productWorkers: { kronos: breakingInputs.worker },
            });
    ...
    describe('in-flight single-flight', () => {
        it('does not double-submit the same jobId when a stale entry sits in inFlight', async () => {
            ...
            const collInputs = makeTickInputs(harness, me, { metrics, inFlight });
            await tickOnce({
                runtime: harness.runtime,
                wallet: harness.fakeWalletFor(me),
                handlers: collInputs.handlers,
                sources: collInputs.sources,
                metrics: collInputs.metrics,
                logger: SILENT_LOGGER,
                checkpointState: emptyCheckpointState(),
                productWorkers: { kronos: collInputs.worker },
            });
    ...
    describe('mirror maintenance across lifecycle', () => {
        it('adds + removes mirror entries as automatons register and unstake', async () => {
            ...
            // Warm the mirror.
            const { kronosWorker, checkpointState } = await runTicks({ harness, me, ticks: 1 });
            const mirror = kronosWorker.mirror;
