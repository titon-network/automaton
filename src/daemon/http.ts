...
export function startHealthServer(options: HealthServerOptions): Promise<HealthServer> {
    ...
    // Conservative timeouts — slowloris-style attacks try to hold
    // sockets open with byte-by-byte header writes. Node 18+ defaults
    // are 60s/60s; we tighten because /metrics scrapes finish in <100ms
    // and /healthz is even smaller. A request that hasn't sent its
    // headers within 5 s, or hasn't completed within 10 s, isn't a
    // legitimate Prometheus scrape.
