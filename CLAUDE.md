...
`@titon-network/automaton` is the **universal off-chain operator node** for the Titon protocol suite on TON. Operators install and run it to stake with ForgeTON (shared-security pool) and earn across every admitted consumer product — Kronos (automation), Fortuna (verifiable randomness), and any future product wired in via `config.products.<name>`.

Per-product responsibilities:
- **Kronos** (baseline): poll the registry, sign+send `Execute` for due jobs, react to slash events.
- **Fortuna** (opt-in via `products.fortuna`): hold a BLS12-381 pkShare registered at Atlas, watch Fortuna's request stream, sign alpha with the BLS secret, and submit `FulfillRandomness` before the deadline. Solo-operator path (t=1, n=1) lands first; multi-operator threshold-BLS aggregation is the follow-up sub-phase.

Design goal: *super easy to launch, hassle-free, production-ready*. `automaton init` → `automaton stake register` → `automaton run` is the Kronos onboarding flow; add `automaton bls keygen` → `automaton bls register` when enabling Fortuna. `automaton doctor` / `automaton status` for diagnostics.

## Tech stack

Node.js ≥22 (preflight-gated), TypeScript 5.5+ strict/NodeNext CJS, `commander` v12, `zod`, `pino` (with structural redaction), `prom-client`, `@ton/core` + `@ton/ton` (`WalletContractV5R1`), `@ton/crypto` for BIP-39, Node's `crypto` for scrypt+AES-256-GCM, BLS12-381 via `@noble/curves/bls12-381` (routed through `@titon-network/fortuna-sdk`), Jest+ts-jest `--runInBand` with 1 GB per-worker heap. `@titon-network/forgeton-sdk` + `@titon-network/kronos-sdk` + `@titon-network/atlas-sdk` + `@titon-network/fortuna-sdk` are sibling `file:` deps — pnpm **copies** them at install, does NOT live-link.
...
> ⚠️ **SDK snapshot staleness.** Edit `../forgeton/sdk/src/` or `../kronos/sdk/src/` → this repo is instantly stale. Run `pnpm run sync:sdks`. `tests/preflight.ts` catches the common failure (missing `dist/`) at test start.
...
| Inject a sandbox-friendly I/O boundary into a worker | `KronosWorker.submitExecute?` (`src/worker/loop.ts`) · `FortunaWorker.sender?` + `FortunaWorker.submitFulfill?` (`src/worker/fortuna.ts`) | Worker constructors accept optional injection points so sandbox callers (the playground, integration tests) can route through `treasury.getSender()` instead of the production V5R1-wallet seqno-poll path. Same pattern across both workers — mirror it for new product workers. |
...
| Change the Fortuna fulfillment path | `src/worker/fortuna.ts` | `FortunaWorker.eventHandler()` keeps the pending queue in sync; `.tick()` iterates pending, signs via `signAlpha`, submits via `sendFulfillRandomness`. Solo-operator today — multi-op threshold aggregation is the next sub-phase |
...
| Add an event handler | `src/worker/handlers.ts` | Write a factory returning `EventHandler`; `onRegistry` / `onPool` / `onFortuna` callbacks receive decoded SDK events + `TxContext`. `drainEvents` orchestrates dispatch. Existing handlers as templates: `mirrorPatchHandler` (debounce-via-onCycleEnd), `selfSlashHandler` (filter on `me`), `automatonAwarenessHandler` (catch-all on events naming this automaton), `registryHealthHandler` (protocol health log), `fortunaConfigInvalidatorHandler` (drop FortunaWorker cache on ConfigUpdated) |
...
| Deploy on AWS | `contrib/aws/lightsail/` | Lightsail-only (~$3.50/mo `nano_3_0`, ~$5/mo `micro_3_0` for the Fortuna leg). Keystore-mode bootstrap (operator runs `automaton init` locally; mnemonic stays off AWS; `wallet.enc` + optional `bls.enc` ride in Terraform state — encrypt the backend). Optional `bls_keystore_file` knob enables Fortuna alongside Kronos. Validated via `terraform validate` + `terraform fmt`. See the module's README for the full quickstart + security trade-offs. For larger mainnet positions where the keystore should not ride in TF state, prefer self-hosted systemd or Docker with a managed-secrets backend. |
...
- `node_modules/@titon-network/forgeton-sdk/`, `node_modules/@titon-network/kronos-sdk/` — snapshot copies from `file:` deps; run `pnpm run sync:sdks` after editing sibling SDK source.
...
8. **`../BLUEPRINT.md`** — workspace-level entity+message-flow diagram. Load for cross-contract scope questions.
...
export const PRODUCTS: readonly ProductModule[] = [fortuna, phoebe];
```
...
Two products supported out of the box:

- **Kronos** (always-on baseline): poll the registry, decide which jobs to execute, submit `Execute`, react to slash events.
- **Fortuna** (opt-in via `config.products.fortuna`): hold a BLS pkShare registered at Atlas, watch Fortuna's request stream, sign alpha, submit `FulfillRandomness` before the deadline. Solo-operator (t=1, n=1) today; threshold-BLS aggregation across peers is a future extension that keeps the on-chain shape unchanged.

Operator-adverse conditions (slashed, pruned, missed assigned slot, group-key rotation, registry/pool/fortuna pause flips, code upgrades, opt-in changes) surface at the appropriate log level via the built-in event handlers — the operator doesn't grep tx history to find out what happened.
