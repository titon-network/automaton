...
4. **Just Kronos, or also Fortuna VRF?** Kronos is the baseline (any active staked operator participates in round-robin assignment automatically). Fortuna is opt-in via `config.products.fortuna`. Two onboarding shapes:
   - **Solo-mode (testnet only)** — `t=1, n=1`, `pkShare == groupPk` invariant. Single operator. Atlas's `publishSoloGroupKey` ceremony script. Single point of forgery; deliberately blocked on mainnet.
