...
// consumes. Exercise the happy-path shape on both networks now that
// mainnet is live.

import { buildChainRuntime } from '../src/chain';
...
import { FORGETON_MAINNET } from '@titon-network/forgeton-sdk';
...
describe('buildChainRuntime', () => {
    ...
    it('assembles a runtime for mainnet — kronos contributes registry, pool is baseline', () => {
        const runtime = buildChainRuntime(defaultConfig('mainnet'));
        expect(runtime.client).toBeInstanceOf(FailoverTonClient);
        expect(runtime.client.currentEndpoint).toBe('https://toncenter.com/api/v2/jsonRPC');
        expect(runtime.deployment.products.kronos?.registry?.equals(KRONOS_MAINNET.registry)).toBe(true);
        expect(runtime.deployment.pool.equals(FORGETON_MAINNET.forgeton)).toBe(true);
    });
