...
export const phoebe: ProductModule = {
    ...
    async bootstrapWorker(ctx): Promise<ProductWorker | undefined> {
        ...
        // takes the fast path (no server, no cache, no public read
        // endpoint — solo operators aren't part of a decentralized
        // indexer set anyway).
        //
        // The public read endpoint (GET /phoebe/v1/snapshot) rides on
        // the SAME server as the multi-op share endpoint. Multi-op
        // operators get it free; consumers reach them via the same
        // port they already expose. Bind to a public interface by
        // setting `config.phoebe.shareExchangeHost: "0.0.0.0"`.
        const snapshotRef: PhoebeSnapshotRef = { current: null };
