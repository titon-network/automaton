...
export function resolveDeployment(config: Config): Deployment {
    ...
    switch (config.network) {
        ...
        case 'mainnet':
            throw new DeploymentNotAvailableError(
                'ForgeTON mainnet deployment is not yet live. Testnet addresses are in ' +
                    '@titon-network/forgeton-sdk as FORGETON_TESTNET; when mainnet lands a ' +
                    'FORGETON_MAINNET constant will ship in the same place.',
            );
    }
