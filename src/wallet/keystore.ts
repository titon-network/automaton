...
// trailing nibble, so an odd-length string would parse but yield wrong
// bytes. Refuse loudly instead of decrypting with truncated material.
const HEX = /^([0-9a-f]{2})+$/i;

// Bound on scrypt cost — a tampered keystore with N=2^30 would lock the
// process for hours during unlock. 2^20 is well above the production
// default (2^17 ≈ 300-500 ms) and below the DoS threshold (2^21+ pushes
// past several seconds even on fast hardware).
