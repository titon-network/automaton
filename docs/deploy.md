...
| "Cheapest cloud" / "Lightsail" / "VPS" / "AWS" | [§3 AWS Lightsail](#3-aws-lightsail--cheapest-cloud) | ~3 min apply + 10 min faucet (after local wallet init) | ~$3.50/mo Kronos · ~$5/mo Fortuna |
| "Docker" / "my own server" | [§4 Self-hosted Docker](#4-self-hosted-docker--vps-or-bare-metal) | ~10 min + faucet | depends on host |
| "systemd" / "bare-metal Linux" | [§5 Self-hosted systemd](#5-self-hosted-systemd--from-source-on-linux) | ~15 min + faucet | depends on host |

**Default questions to ask the user before picking** (only if the path isn't obvious):
1. **Testnet or mainnet?** Lightsail (§3) keystore-mode lands the encrypted `wallet.enc` in Terraform state + Lightsail user-data — fine for testnet and small mainnet positions. For larger mainnet positions where the keystore should NOT ride in TF state, route to §4 (Docker) or §5 (systemd) with a managed-secrets backend of your choice.
2. **Do they have AWS already configured?** `aws configure` works + has IAM permissions → §3. Otherwise → §1 / §2 / §4 / §5.
3. **Trying it or operating it?** Trying → §1 Playground (no real TON, exit Ctrl-C). Operating → any path §2–§5 (real wallet, real chain).
4. **Just Kronos, or also Fortuna VRF?** Kronos is the baseline; Fortuna is opt-in. Fortuna onboarding has one extra prerequisite — Atlas (the BLS group-key substrate) must be admitted as a ForgeTON consumer (one-time, ForgeTON-owner-driven). If you (the deployer) aren't the ForgeTON owner, the operator's `automaton bls register` will revert with `OperatorNotFound (120)` until that step lands. See [§Enabling Fortuna VRF](#enabling-fortuna-vrf) below.
...
To run as a service (Linux): see [§5 Self-hosted systemd](#5-self-hosted-systemd--from-source-on-linux). For Docker: see [§4](#4-self-hosted-docker--vps-or-bare-metal).

---

## 3. AWS Lightsail — cheapest cloud
...
## 4. Self-hosted Docker — VPS or bare metal
...
## 5. Self-hosted systemd — from source on Linux
