...
locals {
  ...
  # Phoebe is enabled iff the operator listed at least one feed AND a BLS
  # keystore is present (same Atlas group secret signs Fortuna fulfillments,
  # Themis reveals, and Phoebe pushes). Feed entries have a union type
  # (static OR dynamic) which Terraform can't express cleanly, so we accept
  # `any` at the variable boundary and round-trip through JSON. base64 wraps
  # the JSON for safe heredoc interpolation regardless of feed payload —
  # base64 alphabet has no bash-special characters.
  enable_phoebe         = length(var.phoebe_feeds) > 0 && var.bls_keystore_file != null
  phoebe_feeds_json_b64 = base64encode(jsonencode(var.phoebe_feeds))
...
resource "aws_lightsail_instance" "this" {
  ...
  user_data = templatefile("${path.module}/user-data.sh.tftpl", {
    ...
    enable_phoebe           = local.enable_phoebe
    phoebe_feeds_json_b64   = local.phoebe_feeds_json_b64
    phoebe_push_interval_ms = var.phoebe_push_interval_ms
  })
