...
variable "phoebe_feeds" {
  description = <<EOT
    List of Phoebe price feeds this operator publishes. Empty list
    (default `[]`) leaves phoebe disabled AND zero overhead — the worker
    only boots when at least one feed is configured. When non-empty, the
    daemon enables `products.phoebe` and writes the feeds under
    `phoebe.feeds` in config.json — the PhoebeWorker heartbeat-pushes a
    BLS-attested Merkle snapshot every `phoebe_push_interval_ms` window.

    Each entry is EITHER static (operator hand-sets the price) — the
    only path that gives byte-identical roots across operators in
    multi-op:
        { feedId = 0, mantissa = "300", expo = -2, confBps = 50 }
    OR dynamic (aggregator pulls live prices from CEX websockets +
    DEX TWAP fallback, takes the median across fresh ticks):
        { feedId = 0,
          sources = [
          ...
          expo = -8 }
    Known adapter names: `binance`, `coinbase`, `kraken`, `stonfi-twap`.

    Phoebe additionally requires `bls_keystore_file` — phoebe reuses the
    Fortuna/Themis BLS share (the Atlas group secret signs Fortuna
    fulfillments, Themis reveals, AND Phoebe snapshot pushes — one
    `bls.enc` covers all three).
  EOT
  type        = any
  default     = []
}

variable "phoebe_push_interval_ms" {
  description = <<EOT
    Phoebe snapshot push cadence in milliseconds. Default 30000 (30s)
    matches the automaton schema default and the contract's typical
    `maxPushDrift`. Floor 5000 (below = gas waste), ceiling 3600000
    (above = consumers'  `maxStaleness` checks start failing).
  EOT
  type        = number
  default     = 30000
}
