...
export class PhoebeWorker {
    ...
    private buildTree(): { tree: PhoebeMerkleTree; droppedDynamic: number } {
        ...
        return { tree: PhoebeMerkleTree.fromSparseLeaves(leaves), droppedDynamic };
    }
    ...
    private async submitPushSnapshot(opts: {
        ...
        try {
            ...
            // Post-send verify: a successful push stamps `lastSubmitter` to
            // our wallet. If it shows a different address we lost a race
            // (a peer pushed first under multi-op fallback). Log + treat
            // as success — the next cadence window will push again.
            const verify = async (): Promise<void> => {
                const submitter = await this.phoebe.getLastSubmitter();
                if (submitter !== null && !submitter.equals(this.deps.wallet.address)) {
                    logger.warn(
                        'phoebe: post-push getLastSubmitter shows a different address — peer won the race',
